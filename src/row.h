...
public:
  ...
  // Precondition:
  //	A white or black piece needs to be moved from the row. Number of current
  // pieces and the number of the row are required. Postcondition: 	The
  // position of the moved piece is replaced with a < or > as position requires.
  void updateRowAddWhite(int l, int r);
  // Purpose:
  //	To visually update the addition of a white piece to a row.
  // Precondition:
  //	A white piece needs to be moved to the row. Number of current pieces and
  // the number of the row are required. Postcondition: 	A W is added to
  // the row in the appropriate position based on how many are currently there.
  void updateRowAddBlack(int l, int r);
  // Purpose:
  //	To visually update the addition of a black piece to a row.
  // Precondition:
  //	A black piece needs to be moved to the row. Number of current pieces and
  // the number of the row are required. Postcondition: 	A B is added to
  // the row in the appropriate position based on how many are currently there.
  void incrementWhite();
  // Purpose:
  ...
  void setRow(int x);
  // Purpose:
  //	Sets up the necessary initial contents of the rows to form the starting
  // gameboard. Precondition: 	The row's numbered position on the board needs
  // to be passed to it. Postcondition: 	Rows are properly assigned based
  // on their labeled position on the game board.
  void printRow();
  // Purpose:
